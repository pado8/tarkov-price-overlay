...
> **다른 세션에서 이어서 작업할 때: [PLAN.md](PLAN.md)를 먼저 읽을 것.** 세션 제약(로컬 전용·push 금지), 3.29 조사 결과 전문, 데이터 주입 런북, 함정 목록이 거기 있다.

- `/` 홈 — 도구 3종 카드
- `/enshrouding` — 인슈라우딩 시뮬레이터 (군단 리워크 유니크 변환)
- `/allflame` — 올플레임 크래프팅 시뮬레이터: 인게임 Ctrl+C 아이템 붙여넣기 → 화폐/두캇 적용 → 고스트 미리보기 택1 (베스퍼·무형화·유황·두캇, 신규 모드는 간이 풀 근사)
