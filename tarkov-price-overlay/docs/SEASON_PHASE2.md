...
> **상태 갱신 (같은 날): 2단계 구현 완료 — dev `60b37b4`.** 아래 체크리스트
> 1~6 전부 반영. 검증 = 단위 32건(분류·정규식·혼합 방어·상태 마이그레이션) +
> 빌드·run-local 스모크(/quests/status에 season·mixed_mode_folders 필드,
> by_mode에 season 키, 시즌 시세 분리 유지). **혼합 세션은 가설이 아니라
> 실재로 승격**: 이 PC 실로그 323폴더 중 3개에서 한 application 로그에
> `Regular`+`Pve` 두 라인 실검출(최초 2026-02 — 시즌 이전부터 있던 현상.
> 구코드는 첫 라인 기준으로 오귀속했고, mixed 스킵이 그 잠재 버그까지 차단).
> **남은 것 = 사용자 실로그 검증**(아래 절차): 시즌 캐릭터 선택 후
> `Session mode: PvpSeason` 라인과 앱의 season 카운트 증가 확인.

1단계(시즌 시세·바터·제작·은신처, `a93c5bd`)는 출하됨. 이 문서는 2단계
