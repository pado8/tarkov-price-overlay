...
- **v1.2.2** — **멀티모니터 hover 수정** (피드백): 배율(DPI)이 서로 다른 듀얼·멀티 모니터에서 오버레이 카드 위에 마우스를 올렸을 때 클릭 투과(hover) 판정 경계가 어긋나던 문제 수정 + **백그라운드 통신 추가 감소** (익명 통계 전송 주기 조정 — 사용자 체감 변화 없이 서버 리소스 절약)
...
- **v1.2.2** — **Multi-monitor hover fix** (feedback): on dual/multi-monitor setups with different scaling (DPI), fixed the click-through (hover) boundary being miscalculated over the overlay card + **fewer background calls** (anonymous-stats send interval tuned — saves server resources with no user-facing change)
