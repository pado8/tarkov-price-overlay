...
- **v1.1.4** — **사용자 피드백 5건 반영**: **퀘스트 잠금 바터 표시** (특정 퀘스트를 깨야 풀리는 교환이 그냥 가능한 것처럼 보이던 문제 — 이제 🔒 퀘스트명 배지로 표시되고, 퀘스트 트래커 연동 시 완료한 퀘스트는 초록색 ✓로 바뀜) + **은신처 제작 미리보기** (시설 레벨이 모자라 아직 못 하는 제작도 흐리게 + "Lv N 필요" 배지로 계속 보여줘 미래 계획 가능) + **풀네임 호버 툴팁** (6B5-16, MBSS 같은 약칭에 마우스를 올리면 전체 이름 표시 — 메인 카드·교환·제작·탄약표 전부) + **탄약 팩 하이라이트** (M855A1 박스 등 포장 탄약을 조회하면 탄약표에서 그 탄이 어느 행인지 표시) + **거래창 OCR 정확도 개선** (아이템 이름 아래 가격(₽)·"구매 불가" 문구가 같이 잡혀 조회를 망치던 것을 자동으로 걸러냄)
