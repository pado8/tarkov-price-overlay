...
- **v1.2.1** — **와이프 리셋** (설정 → 퀘스트 동기화): 타르코프 와이프·시즌 시작으로 인게임 퀘스트가 초기화됐을 때, 이 시점 이전의 옛 로그 기록을 무시하고 새로 동기화 — 지난 시즌 완료 표시가 되살아나던 문제 해결. 실수로 눌러도 '수동 동기화'로 복구 + **시즌 세션 가드** (7월 시즌제 대비 — 새 게임 모드의 퀘스트 이벤트가 영구 PVP 진행도를 오염시키지 않게 차단)
